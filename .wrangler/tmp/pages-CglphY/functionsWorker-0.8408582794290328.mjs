...
// ../.wrangler/tmp/bundle-w91eGH/strip-cf-connecting-ip-header.js
...
async function onRequest(context) {
  ...
  for (let year = startYear; year <= endYear; year++) {
    try {
      ...
      const existingValue = await context.env.HOLIDAYS_KV.get(year.toString());
      if (existingValue === newValue) {
        results.push({ year, status: "skipped", message: "Data is already identical, KV write skipped" });
      } else {
        await context.env.HOLIDAYS_KV.put(year.toString(), newValue);
        if (cache) {
          try {
            ...
            results.push({ year, status: "updated", message: "KV updated and edge cache purged" });
          } catch (cacheDeleteError) {
            results.push({ year, status: "updated", message: `KV updated, cache purge failed: ${cacheDeleteError.message}` });
          }
        } else {
          results.push({ year, status: "updated", message: "KV updated, cache not purged (no cache API)" });
        }
      }
    } catch (err) {
...
async function onRequest2(context) {
  ...
  try {
    const value = await context.env.HOLIDAYS_KV.get(year);
    if (!value) {
...
// ../.wrangler/tmp/pages-CglphY/functionsRoutes-0.9434031551914694.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-w91eGH/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-w91eGH/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8408582794290328.mjs.map
